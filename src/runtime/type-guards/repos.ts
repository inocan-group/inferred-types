import { IsTrue, OptionalSpace, RepoSource, RepoUrls, SemanticVersion, UrlsFrom } from "src/types/index";
...
): v is T & SemanticVersion<IsTrue<P> extends true ? `${"v" | ""}${OptionalSpace}` : ""> => {
  return (
    isString(v) &&
    v.split(".").length === 3 &&
