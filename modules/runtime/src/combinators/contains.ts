import { Contains, Narrowable } from "inferred-types/types";
...
>(
  ...
  const valueMatches = (value: unknown, comp: unknown): boolean => {
    ...
    if (comp === String) return typeof value === 'string';
    if (comp === Number) return typeof value === 'number';
    if (comp === Boolean) return typeof value === 'boolean';

    // Special handling for null/undefined
    if (comp === null) return value === null;
    if (comp === undefined) return value === undefined;
