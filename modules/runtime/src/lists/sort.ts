...
export function sort<
 ...
 {
     if (firstValues.length === 0) return false;
     ...
     for (const value of firstValues) {
         ...
         if (offset && typeof item === 'object' && item !== null) {
             ...
             if (objItem[offset] === value) return true;
         }
         // For string tokens like "String(bar)", extract the actual value
             ...
             if (match) {
                 ...
                 if (type === 'String' && typeof item === 'string' && item === val) return true;
                 if (type === 'Number' && typeof item === 'number' && item === Number(val)) return true;
             }
         }
         // Direct comparison
         else if (item === value) {
