...
type UnionComparison<
    ...
    TElse,
> = Or<{
    [K in keyof TTarget]: Compare<TTarget[K], TOp, TComparator> extends boolean
        ? Compare<TTarget[K], TOp, TComparator>
        : false
}> extends true
    ? TIf
    : TElse;
...
/**
 * **SomeUnionElement**`<TTarget, TOp, TComparator, [TIf], [TElse]>`
 *
 * When `TTarget` is a union type:
 *
 *    - compares the elements of `TTarget` using the `TOp` comparison operator to the
 * `TComparator` type
 *    - by default returns boolean true/false based on whether _any_ element in the union
 ...
 *
 * **Related:** `EveryUnionElement`
 */
export type SomeUnionElement<
    TTarget,
    TOp extends ComparisonOperation,
    TComparator extends Flexy<ComparisonLookup[TOp]["params"]>,
    TIf = true,
