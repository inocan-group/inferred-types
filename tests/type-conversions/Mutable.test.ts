...
describe("Mutable<T>", () => {
    it("object with scalar values", () => {
        ...
        type cases = [Expect<AssertEqual<T1, { foo: 1; bar: 2 }>>];
    });
...
describe("mutable", () => {
    it("object with scalar values", () => {
        const fooBar = mutable({ foo: 1, bar: 2 } as {
            readonly foo: 1;
            readonly bar: 2;
        });

        type cases = [Expect<AssertEqual<typeof fooBar, { foo: 1; bar: 2 }>>];
    });

    it("object with readonly arrays", () => {
        const fooBar = mutable({ foo: [1, 2, 3], bar: 2 } as {
            readonly foo: [1, 2, 3];
            readonly bar: 2;
        });
