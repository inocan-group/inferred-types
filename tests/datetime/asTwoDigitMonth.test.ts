...
import { AsTwoDigitMonth, Expect, Test } from "inferred-types/types";

describe("asTwoDigitMonth() runtime function", () => {
    ...
    it("handles Date objects", () => {
        ...
        // type tests - Date objects at runtime resolve to AsTwoDigitMonth<Date>
        // which results in an error since Date is not parseable at compile time
        type cases = [
            Expect<Test<typeof jan, "isError", "invalid-date/as-two-digit-month">>,
            Expect<Test<typeof jul, "isError", "invalid-date/as-two-digit-month">>,
            Expect<Test<typeof dec, "isError", "invalid-date/as-two-digit-month">>,
        ];
    });
...
describe("AsTwoDigitMonth<T> type utility", () => {
    ...
    it("handles ISO datetime string literals", () => {
        type T1 = AsTwoDigitMonth<"2024-01-15T10:30:00Z">;
        type T5 = AsTwoDigitMonth<"2024-05-01T00:00:00Z">;
        type T11 = AsTwoDigitMonth<"2024-11-30T12:00:00Z">;
        
        type cases = [
            Expect<Test<T1, "equals", "01">>,
            Expect<Test<T5, "equals", "05">>,
            Expect<Test<T11, "equals", "11">>,
        ];
    });
    
    it("handles partial ISO dates", () => {
        // Note: The type utility may not support all partial formats
        // These are expected to error since they're not valid ISO dates
        type T1 = AsTwoDigitMonth<"2024-03">;
        type T2 = AsTwoDigitMonth<"--03-15">;
        type T3 = AsTwoDigitMonth<"--04-30">;
        
        type cases = [
            Expect<Test<T1, "isError", "invalid-date/as-two-digit-month">>,
            Expect<Test<T2, "isError", "invalid-date/as-two-digit-month">>,
            Expect<Test<T3, "isError", "invalid-date/as-two-digit-month">>,
        ];
    });
    ...
    it("handles branded option", () => {
        ...
        // Branded types should have brand property
        type cases = [
            // Normal (unbranded) should be plain strings
            Expect<Test<T1Normal, "equals", "01">>,
            Expect<Test<T2Normal, "equals", "05">>,
            // Branded versions include brand information
            Expect<Test<T1, "extends", string>>,
            Expect<Test<T2, "extends", string>>,
            Expect<Test<T3, "extends", string>>,
        ];
    });
    
    it("handles edge cases for numeric literals", () => {
        type T0 = AsTwoDigitMonth<0>; // Outside 1-12, treated as timestamp
        type T13 = AsTwoDigitMonth<13>; // Outside 1-12, treated as timestamp
        type TNeg = AsTwoDigitMonth<-1>; // Negative, treated as timestamp
        
        type cases = [
            // These should resolve to valid month strings since they're treated as timestamps
            Expect<Test<T0, "extends", string>>,
            Expect<Test<T13, "extends", string>>,
            Expect<Test<TNeg, "extends", string>>,
        ];
    });
    ...
    it("handles wide types appropriately", () => {
        type TString = AsTwoDigitMonth<string>;
        type TNumber = AsTwoDigitMonth<number>;
        type TDate = AsTwoDigitMonth<Date>;
        
        type cases = [
            // Wide types result in errors since they can't be parsed at compile time
            Expect<Test<TString, "isError", "invalid-date/as-two-digit-month">>,
            Expect<Test<TNumber, "extends", string>>, // number is DateLike
            Expect<Test<TDate, "isError", "invalid-date/as-two-digit-month">>,
        ];
    });
});