...
import { Keys, NumericKeys } from "src/types/Keys";
  ...
  it("happy path", () => {
    ...
    type cases = [
      ...
      Expect<ExpectExtends<
        (readonly ["foo", "bar"] | readonly ["bar", "foo"]) & (keyof Obj)[], 
        Foobar
      >>,
      Expect<ExpectExtends<
        (readonly ["foo", "bar"] | readonly ["bar", "foo"]) & (keyof Obj_RO)[], 
        RoFoobar
      >>,
    ];
...
describe("keys() utility on object", () => {
  ...
  it("keys of a VueJS Ref<T>", () => {
    ...
    expect(
      (k as any[]).includes("__v_isRef"),
      `VueJS ref looks like: ${k}`
    ).toBe(true);

    type cases = [
      Expect<ExpectExtends<
        (readonly [symbol, "value"] & (keyof typeof obj)[]) |
        (readonly ["value", symbol] & (keyof typeof obj)[]), 
        typeof k
      >>
    ];
    const cases: cases = [ true ];
