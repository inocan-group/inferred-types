...
import type { Expect, IsBalanced, Test } from "inferred-types/types";
...
describe("IsBalanced<T,U>", () => {
    ...
    it("greater-than and less-than chars", () => {
        type Basic = IsBalanced<
            "<1> monkey went to the market, <0> bacon was purchased",
            { "<": ">" }
        >;
        type Nested = IsBalanced<
            "<1 or <2>> monkey went to the market, <0> bacon was purchased",
            { "<": ">" }
        >;
        type OuterNesting = IsBalanced<
            "<<1> monkey went to the market, <0> bacon was purchased>",
            { "<": ">" }
        >;

        type Imbalanced = IsBalanced<
            "<<1> monkey went to the market, <0> bacon was purchased",
            { "<": ">" }
        >;
